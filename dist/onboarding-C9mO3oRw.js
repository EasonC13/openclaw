...
import { Pg as readConfigFileSnapshot, Rg as writeConfigFile } from "./auth-profiles-GWyaossq.js";
...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./auth-profiles-GWyaossq.js").then((n) => n.Ct);
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DLnEsXxF.js").then((n) => n.t);
		const { logConfigUpdated } = await import("./logging-FkwajLEi.js").then((n) => n.r);
	...
	const { ensureAuthProfileStore } = await import("./auth-profiles.runtime-lbhskZdB.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BJlMkprz.js").then((n) => n.t);
	const { promptCustomApiConfig } = await import("./onboard-custom-BuYpMQTq.js").then((n) => n.r);
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-_QIIfvMH.js").then((n) => n.t);
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-CV5JIcIw.js").then((n) => n.i);
	...
	const { configureGatewayForOnboarding } = await import("./onboarding.gateway-config-ED13fSok.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-CWeA1EMa.js").then((n) => n.n);
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BT7zY0Mm.js").then((n) => n.t);
	...
	await writeConfigFile(nextConfig);
	const { finalizeOnboardingWizard } = await import("./onboarding.finalize-CHixofZh.js");
