...
import { Eg as installUnhandledRejectionHandler, L_ as loadDotEnv } from "./auth-profiles-Bhj7KY_0.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-C-M-F6nc.js";
import { t as assertSupportedRuntime } from "./runtime-guard-CPrdsppg.js";
import { t as emitCliBanner } from "./banner-CSLWci8w.js";
import "./doctor-config-flow-Bu73-78W.js";
import { n as ensureConfigReady } from "./config-guard-BQ65qAqp.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-E1kYF2ug.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-BVQl32im.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-DNF1iKkj.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-X-mYxNAL.js");
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./auth-profiles-Bhj7KY_0.js").then((n) => n.V);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-Dw2H-Yte.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-Dw2H-Yte.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-BlLt91Ky.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-BlLt91Ky.js");
...
async function closeCliMemoryManagers() {
	try {
		const { closeAllMemorySearchManagers } = await import("./search-manager-CXibZwMQ.js").then((n) => n.n);
...
async function runCli(argv = process$1.argv) {
	...
	try {
		...
		const { buildProgram } = await import("./program-BZn_O9QE.js");
		...
		if (primary) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-PZMO8eE5.js").then((n) => n.t);
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-Dhm1DPYy.js").then((n) => n.a);
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-Dhm1DPYy.js").then((n) => n.a);
