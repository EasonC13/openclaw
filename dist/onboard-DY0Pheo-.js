...
import { At as randomToken, Ct as handleReset, Ft as waitForGatewayReachable, Tt as normalizeGatewayTokenInput, __ as writeConfigFile, _t as applyWizardMetadata, bt as ensureWorkspaceAndSessions, gt as DEFAULT_WORKSPACE, jt as resolveControlUiLinks, p_ as readConfigFileSnapshot } from "./reply-wyWTjlY3.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-oU6etstQ.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-CBY3fY-Y.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-lm0Bmizz.js").then((n) => n.i);
