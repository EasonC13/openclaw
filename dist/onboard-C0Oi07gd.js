...
import { m as restoreTerminalState, p as defaultRuntime } from "./subsystem-DBBM_0ww.js";
import { y as resolveUserPath } from "./utils-JhJvOE-Q.js";
import { At as resolveControlUiLinks, C_ as readConfigFileSnapshot, D_ as writeConfigFile, Pt as waitForGatewayReachable, St as handleReset, gt as applyWizardMetadata, ht as DEFAULT_WORKSPACE, kt as randomToken, wt as normalizeGatewayTokenInput, yt as ensureWorkspaceAndSessions } from "./reply-DCmI6_Pe.js";
...
import { i as normalizeLegacyOnboardAuthChoice, r as isDeprecatedAuthChoice, t as ONBOARD_PROVIDER_AUTH_FLAGS } from "./onboard-provider-auth-flags-MPWD5mzk.js";
import { t as applyOnboardingLocalWorkspaceConfig } from "./onboard-config-BWpRw_IO.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-DAxGx_XF.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-BADf2__X.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-BWL7KB1K.js").then((n) => n.i);
		...
		if (!probe.ok) {
			const message = [
				...
				!opts.installDaemon ? [
					...
					process.platform === "win32" ? "Native Windows managed gateway install currently uses Scheduled Tasks and may require running PowerShell as Administrator." : void 0
				].filter(Boolean).join("\n") : void 0
			].filter(Boolean).join("\n");
